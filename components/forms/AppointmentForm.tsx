...
export const AppointmentForm = ({
  ...
  const onSubmit = async (values: AppointmentFormValues) => {
    ...
    try {
      if (type === "create" && patientId) {
        ...
        if (!appointment?.$id) {
          throw new Error("Appointment ID is missing for update/cancel.");
        }
  ...
  return (
    ...
    >
      ...
      <SubmitButton
        isLoading={isLoading}
        className={`${type === "cancel" ? "bg-red-700 text-white" : "bg-green-500 text-white"} w-full cursor-pointer`}
      >
        {buttonLabel}
      </SubmitButton>
    </form>
